...
$app->post('/getWineSoldWeekly', function() use ($app) {
    ...
    $query = "SELECT 
    IFNULL(SUM(IF(period = 0, sold, 0)),0) AS 'Lunedì',
    IFNULL(SUM(IF(period = 1, sold, 0)),0) AS 'Martedì',
    IFNULL(SUM(IF(period = 2, sold, 0)),0) AS 'Mercoledì',
    IFNULL(SUM(IF(period = 3, sold, 0)),0) AS 'Giovedì',
    IFNULL(SUM(IF(period = 4, sold, 0)),0) AS 'Venerdì',
    IFNULL(SUM(IF(period = 5, sold, 0)),0) AS 'Sabato',
    IFNULL(SUM(IF(period = 6, sold, 0)),0) AS 'Domenica',
    IFNULL(SUM(sold),0) AS total
    FROM (
        SELECT SUM(winesold.value) as sold, WEEKDAY(winesold.date) as period FROM winesold, winelist WHERE winelist.sku = winesold.sku ". $subquery ." AND winesold.date >= '". $data->periodStart ."' AND winesold.date <= '". $data->periodEnd ."' GROUP BY WEEKDAY(winesold.date)
    ) AS SubTable1";
    ...
    IFNULL(SUM(sold),0) AS total
    FROM (
        SELECT (SUM(winesold.value)*winelist.suggested_price) as sold, WEEKDAY(winesold.date) as period FROM winesold, winelist WHERE winelist.sku = winesold.sku ". $subquery ." AND winesold.date >= '". $data->periodStart ."' AND winesold.date <= '". $data->periodEnd ."' GROUP BY WEEKDAY(winesold.date), winesold.sku
    ) AS SubTable1";
    ...
    if($_SESSION['role'] == "admin") {
        $query = "SELECT 
                    ...
                    IFNULL(SUM(sold),0) AS total
                    FROM (
                        SELECT (SUM(winesold.value)*winelist.price) as sold, WEEKDAY(winesold.date) as period FROM winesold, winelist WHERE winelist.sku = winesold.sku ". $subquery ." AND winesold.date >= '". $data->periodStart ."' AND winesold.date <= '". $data->periodEnd ."' GROUP BY WEEKDAY(winesold.date), winesold.sku
                    ) AS SubTable1";
    ...
    $response['revenueTws'] = $revenueTws;
...
$app->post('/getWineSoldMonthly', function() use ($app) {
    ...
    if(isset($data->uid) && $data->uid== 0 || !isset($data->uid)) { // all user
        ...
        $subquery = "AND winesold.id_winelist = ". $session["uid"] ." AND winelist.id_user = ". $session["uid"] ."";
    }

    $query = "SELECT 
                IFNULL(SUM(IF(period = 1, sold, 0)),0) AS 'Dall 1 al 7',
                IFNULL(SUM(IF(period = 2, sold, 0)),0) AS 'Dall 8 al 14',
                IFNULL(SUM(IF(period = 3, sold, 0)),0) AS 'Dall 15 al 21',
                IFNULL(SUM(IF(period = 4, sold, 0)),0) AS 'Dall 22 al 28',
                IFNULL(SUM(IF(period = 5, sold, 0)),0) AS 'Dall 28 al 31',
                IFNULL(SUM(sold),0) AS total
                FROM (
                    SELECT SUM(winesold.value) as sold, WEEK(winesold.date,5) - 
            WEEK(DATE_SUB(winesold.date, INTERVAL DAYOFMONTH(winesold.date)-1 DAY),5)+1 as period FROM winesold, winelist WHERE winelist.sku = winesold.sku ". $subquery ." AND winesold.date >= '". $data->periodStart ."' AND winesold.date <= '". $data->periodEnd ."' GROUP BY WEEK(winesold.date,5) - 
            WEEK(DATE_SUB(winesold.date, INTERVAL DAYOFMONTH(winesold.date)-1 DAY),5)+1
                ) AS SubTable1";
    $bottles = $db->getRecord($query);
    ...
    $query = "SELECT 
                ...
                IFNULL(SUM(sold),0) AS total
                FROM (
                    SELECT (SUM(winesold.value)*winelist.suggested_price) as sold, WEEK(winesold.date,5) - 
            WEEK(DATE_SUB(winesold.date, INTERVAL DAYOFMONTH(winesold.date)-1 DAY),5)+1 as period FROM winesold, winelist WHERE winelist.sku = winesold.sku ". $subquery ." AND winesold.date >= '". $data->periodStart ."' AND winesold.date <= '". $data->periodEnd ."' GROUP BY WEEK(winesold.date,5) - 
            WEEK(DATE_SUB(winesold.date, INTERVAL DAYOFMONTH(winesold.date)-1 DAY),5)+1, winesold.sku
                ) AS SubTable1";
    $revenue = $db->getRecord($query);
    ...
    if($_SESSION['role'] == "admin") {
        $query = "SELECT 
                    ...
                    IFNULL(SUM(sold),0) AS total
                    FROM (
                        SELECT (SUM(winesold.value)*winelist.price) as sold, WEEK(winesold.date,5) - 
                WEEK(DATE_SUB(winesold.date, INTERVAL DAYOFMONTH(winesold.date)-1 DAY),5)+1 as period FROM winesold, winelist WHERE winelist.sku = winesold.sku ". $subquery ." AND winesold.date >= '". $data->periodStart ."' AND winesold.date <= '". $data->periodEnd ."' GROUP BY WEEK(winesold.date,5) - 
                WEEK(DATE_SUB(winesold.date, INTERVAL DAYOFMONTH(winesold.date)-1 DAY),5)+1, winesold.sku
                    ) AS SubTable1";
        $revenueTws = $db->getRecord($query);
...
$app->post('/getWineSoldYearly', function() use ($app) {
    ...
    $query = "SELECT 
                IFNULL(SUM(IF(period = 1, sold, 0)),0) AS 'Gennaio',
                IFNULL(SUM(IF(period = 2, sold, 0)),0) AS 'Febbraio',
                IFNULL(SUM(IF(period = 3, sold, 0)),0) AS 'Marzo',
                IFNULL(SUM(IF(period = 4, sold, 0)),0) AS 'Aprile',
                IFNULL(SUM(IF(period = 5, sold, 0)),0) AS 'Maggio',
                IFNULL(SUM(IF(period = 6, sold, 0)),0) AS 'Giugno',
                IFNULL(SUM(IF(period = 7, sold, 0)),0) AS 'Luglio',
                IFNULL(SUM(IF(period = 8, sold, 0)),0) AS 'Agosto',
                IFNULL(SUM(IF(period = 9, sold, 0)),0) AS 'Settembre',
                IFNULL(SUM(IF(period = 10, sold, 0)),0) AS 'Ottobre',
                IFNULL(SUM(IF(period = 11, sold, 0)),0) AS 'Novembre',
                IFNULL(SUM(IF(period = 12, sold, 0)),0) AS 'Dicembre',
                IFNULL(SUM(sold),0) AS total
                FROM (
                    SELECT SUM(winesold.value) as sold, MONTH(winesold.date) as period FROM winesold, winelist WHERE winelist.sku = winesold.sku ". $subquery ."  AND winesold.date >= '". $data->periodStart ."' AND winesold.date <= '". $data->periodEnd ."' GROUP BY MONTH(winesold.date)
                ) AS SubTable1";
    $bottles = $db->getRecord($query);
    ...
    $query = "SELECT 
                ...
                IFNULL(SUM(sold),0) AS total
                FROM (
                    SELECT (SUM(winesold.value)*winelist.suggested_price) as sold, MONTH(winesold.date) as period FROM winesold, winelist WHERE winelist.sku = winesold.sku ". $subquery ."  AND winesold.date >= '". $data->periodStart ."' AND winesold.date <= '". $data->periodEnd ."' GROUP BY MONTH(winesold.date), winesold.sku
                ) AS SubTable1";
    $revenue = $db->getRecord($query);
    ...
    if($_SESSION['role'] == "admin") {
        $query = "SELECT 
                    ...
                    IFNULL(SUM(sold),0) AS total
                    FROM (
                        SELECT (SUM(winesold.value)*winelist.price) as sold, MONTH(winesold.date) as period FROM winesold, winelist WHERE winelist.sku = winesold.sku ". $subquery ." AND winesold.date >= '". $data->periodStart ."' AND winesold.date <= '". $data->periodEnd ."' GROUP BY MONTH(winesold.date), winesold.sku
                    ) AS SubTable1";
        $revenueTws = $db->getRecord($query);
    }
